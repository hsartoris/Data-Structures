...
public class Histogram extends JFrame implements ActionListener {
	...
	class GraphPanel extends JPanel {
		...
		private void drawBars(Graphics g) {
			...
			if (bindat == null || barwidth < 1) {
				g.drawString(String.format("%s", "Cannot display: increase window width"), basex,
						getHeight() - basey / 3);
				return;
			}

			double ymax = getMax(bindat);
			// System.out.println("width " + getWidth() + " barwidth " +
			// barwidth);
			if (ymax <= 0.0)
				ymax = 1.0;
			...
			g.drawString(String.format("%5.3f", getMax(data)), xoffset + nbars * (gap + barwidth),
					getHeight() - basey / 3);
	...
	public void actionPerformed(ActionEvent e) {
		...
		/*
		 * if (numkeys.isNumkey(cmd)) { if (newEntry) { // new number started
		 * total.setText(e.getActionCommand()); newEntry = false; } else {
		 * total.setText(total.getText() + e.getActionCommand()); } }
		 * 
		 * else { System.out.println("Command unknown: " + cmd); }
		 */
	}

	/**
	 * @param dat
	 *            data to make histogram from
